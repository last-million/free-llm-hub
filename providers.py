...
def is_model_allowed(model_id: Optional[str]) -> bool:
    ...
    An opt-in "uncensored only" mode existed here briefly (2026-08-30) and was
    removed the same day. It inverted this filter, so the routable pool became
    the uncensored models ALONE — which on this provider set is 4 models, 3 of
    them on a single per-MINUTE-throttled relay. Any multi-step agent run
    emptied that pool instantly and 503'd, and while the mode was on every
    ordinary model (including the user's own Claude Code subscription) came back
    403. It was a footgun with no working configuration behind it.
    """
    if not model_id:
        return False
    mid = str(model_id)
