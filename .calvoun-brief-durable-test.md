...
Do not start a server or any long-running process from the shell (no `&`, `nohup`, `start`, or a watch mode): the shell tool waits for it and your turn hangs. To check a server, run a one-shot command with a short timeout, or leave it to the hub's preview.
